...
void UART4_RX_TX_IRQHandler(void)
{
	...
	char ch;
	char tof;
	if(uart_query (uart4) == 1)     //查询是否接收到数据
	{
		uart_getchar   (uart4, &ch); 
		tof=ch;  //等待接收一个数据，保存到 ch里
		if(tof != '\n')
		{
			uart_temp[i] = tof;
			i++;
		...
		{
			//      process();
			if(uart_temp[i-2] == 'm'&&uart_temp[i-3] == 'm')
			{
				if(i>3)
					data = (uart_temp[i-4] - 0x30);
				if(i>4)
					data += ((uart_temp[i-5] - 0x30)*10);
				if(i>5)
					data += ((uart_temp[i-6] - 0x30)*100);
				if(i>6)
					data += ((uart_temp[i-7] - 0x30)*1000);
			}
			i = 0;
		}
...
{ 
	uart_rx_irq_dis(uart1);
	int8 ch;
	if(uart_query(uart0)!= 0) //查询接收 1 个字符
	{
