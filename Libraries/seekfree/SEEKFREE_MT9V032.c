...
int16 MT9V032_CFG[CONFIG_FINISH][2]=
{
    ...
    {UD_OFFSET,         75},   //图像上下偏移量    正值 上偏移   负值 下偏移  行为120 240 480时无法设置偏移    摄像头收偏移数据后会自动计算最大偏移，如果超出则设置计算出来的最大偏移
    {GAIN,              16},  //图像增益          范围16-64     增益可以在曝光时间固定的情况下改变图像亮暗程度32
...
///*************************************************************** 
//* 
//* 函数名称：uint8_t GetOSTU(uint8_t tmImage[ROW][COL]) ROW
//* 功能说明：求阈值大小 
//* 参数说明： 
//* 函数返回：阈值大小 
//* 修改时间：2018年3月27日 
...
//***************************************************************/ 
//uint8_t GetOSTU(uint8_t tmImage[ROW][COL]) 
//{ 
//  int16_t i,j; 
//  uint32_t Amount = 0; 
...
///*************************************************************** 
//* 
//* 函数名称：void BinaryImage(uint8_t tmImage[ROW][COL]) 
//* 功能说明：图像数据二值化 
//* 参数说明： 
//* 函数返回：void 
//* 修改时间：2018年3月27日 
//* 备 注： 
//***************************************************************/ 
//void BinaryImage(uint8_t tmImage[ROW][COL],uint8_t ThresholdV) 
//{ 
//  int i = 0, j = 0; 
//  for(i = 0;i < ROW;i++) 
//  { 
